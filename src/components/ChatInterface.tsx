            ...
            <header className="bg-gray-800 border-b border-gray-700 p-4 shadow-md z-10">
                <div className="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        >
                            📂 <span className="font-bold">{fileCount}</span> Docs
                        </button>
                    </div>
                ...
                <div className="max-w-3xl mx-auto space-y-6">
                    {history.length === 0 && (
                        <div className="text-center mt-20 opacity-50 space-y-4">
                            ...
                            <p>Memoria activa con {fileCount} documentos.</p>
                            <p className="text-sm">Pregúntame lo que quieras sobre ellos.</p>
                        </div>
                    )}
                    
                    {/* RENDERIZADO SEGURO DE MENSAJES */}
                    {history.map((msg, idx) => {
                        // Protección contra mensajes corruptos
                        if (!msg || !msg.parts || !msg.parts[0] || !msg.parts[0].text) return null;
            ...
            <div className="p-4 bg-gray-800 border-t border-gray-700">
                <div className="max-w-3xl mx-auto relative">
                    <input 
                        ...
                        className="w-full bg-gray-900 text-white pl-4 pr-12 py-4 rounded-xl border border-gray-600 focus:border-blue-500 outline-none shadow-inner" 
                    />
                    <button onClick={handleSend} disabled={!inputValue.trim() || isQueryLoading} className="absolute right-2 top-2 p-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg disabled:opacity-50">
                        <SendIcon />
                    </button>
                </div>
            </div>
